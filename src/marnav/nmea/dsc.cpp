	...
	: sentence(ID, TAG, talk)
{
	...
	read(*(first + 3), distress_nat_);
	// @todo 4 Type of Communication or Second Telecommand.
	//       I could not find Type of Communication in ITU-R M.493 Table 3

	// @todo 5 Frequency/Channel part. I could not find Table 13
	if ((first + 5)->length() == 10) {
        read(*(first + 5), position_.value());
    }

    // @todo 6 Telephone part
    if((first + 6)->length() == 4) {
        read(*(first + 6), position_time_.value());
    }

	// @todo 7-8 (distress relay)
...
dsc::quadrant dsc::resolve_quadrant(uint8_t quadrant)
{
...
{
	const std::uint8_t quadrant = (address_ / 1000000000) % 10;
	geo::latitude::hemisphere lat_hem = geo::latitude::hemisphere::north;
	geo::longitude::hemisphere lon_hem = geo::longitude::hemisphere::west;
	std::tie(lat_hem, lon_hem) = resolve_quadrant(quadrant);
...
utils::optional<dsc::distress_designation> dsc::get_distress_designation() const
{
	if (cat_ != category::distress) {
		return {};
	}

	return static_cast<distress_designation>(distress_nat_);
}

utils::optional<dsc::first_telecommand> dsc::get_first_telecommand() const
{
	if (cat_ == category::distress) {
		return {};
	}

	return static_cast<first_telecommand>(distress_nat_);
}

utils::optional<geo::position> dsc::get_position() const
{
	if (!position_) {
		return {};
	}

	const std::uint8_t quadrant = (position_.value() / 1000000000) % 10;
	geo::latitude::hemisphere lat_hem = geo::latitude::hemisphere::north;
	geo::longitude::hemisphere lon_hem = geo::longitude::hemisphere::west;
	std::tie(lat_hem, lon_hem) = resolve_quadrant(quadrant);

	std::uint32_t lat = (position_.value() / 100000) % 10000;
	std::uint32_t lon = position_.value() % 100000;

	return geo::position {
		{lat / 100, lat % 100, 0, lat_hem}, {lon / 100, lon % 100, 0, lon_hem}};
}

utils::optional<nmea::time_base> dsc::get_position_time() const
{
    if (!position_time_) {
        return {};
    }

    return time_base{ position_time_.value() / 100, position_time_.value() % 100 };
}
